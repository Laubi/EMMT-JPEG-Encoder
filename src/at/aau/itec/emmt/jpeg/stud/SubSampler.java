...
public class SubSampler implements SubSamplerI {
    ...
    public YUVImageI downSample(YUVImageI yuvImg, int samplingRatio) {
        if(samplingRatio == SubSamplerI.YUV_444){
            return yuvImg; // We do not need down sampling here, so skip it
        }
        ...
        switch (samplingRatio) {
            case SubSampler.YUV_422:
                u = downSample422(u);
                v = downSample422(v);
                break;

            case SubSampler.YUV_420:
                u = downSample420(u);
                v = downSample420(v);
                break;
        }
    ...
    private int[][] downSample422(int[][] data) {
    ...
    private int[][] downSample420(int[][] data) {
